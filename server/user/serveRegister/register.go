...
func NewEtcdRegister(addr string) (*EtcdRegister, error) {
	...
	if err != nil {
		utils.UserLogger.Error("newEtcdRegister failed,error:" + err.Error())
		return nil, err
	}
	e := &EtcdRegister{
		client: cli,
...
func (e *EtcdRegister) CreateLease(expire int64) error {
	...
	if err != nil {
		utils.UserLogger.Debug("create lease failed")
		return err
	}
	e.leaseId = lease.ID
	return nil
...
func (e *EtcdRegister) BindLease(key string, value string) error {
	...
	if err != nil {
		utils.UserLogger.Debug("bind lease failed")
		return err
	}
	return nil
}
func (e *EtcdRegister) KeepAlive() error {
	...
	if err != nil {
		utils.UserLogger.Debug("keep alive failed")
		return err
	}
	go func(resp <-chan *clientv3.LeaseKeepAliveResponse) {
		for {
			select {
			case <-resp:
				utils.UserLogger.Info("keep alive success")
			}
		}
	}(resp)
	return nil
